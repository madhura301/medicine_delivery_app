...
-- =====================================================
-- SECTION 6.2: CREATE CHEMIST PAYOUT ACCOUNT TABLE
-- (EF migration: 20260603183957_AddChemistPayoutAccount)
-- Razorpay Route linked-account / payout configuration per medical store.
-- =====================================================

CREATE TABLE IF NOT EXISTS "ChemistPayoutAccounts" (
    "Id" integer GENERATED BY DEFAULT AS IDENTITY,
    "MedicalStoreId" uuid NOT NULL,
    "RazorpayLinkedAccountId" character varying(50) NULL,
    "RazorpayStakeholderId" character varying(50) NULL,
    "RazorpayProductConfigurationId" character varying(50) NULL,
    "BankAccountNumber" character varying(50) NULL,
    "BankIfscCode" character varying(20) NULL,
    "BankAccountHolderName" character varying(150) NULL,
    "OnboardingStatus" character varying(30) NOT NULL,
    "OnboardingError" character varying(1000) NULL,
    "ActivatedOn" timestamp with time zone NULL,
    "CreatedOn" timestamp with time zone NOT NULL,
    "UpdatedOn" timestamp with time zone NULL,
    CONSTRAINT "PK_ChemistPayoutAccounts" PRIMARY KEY ("Id"),
    CONSTRAINT "FK_ChemistPayoutAccounts_MedicalStores_MedicalStoreId" FOREIGN KEY ("MedicalStoreId") REFERENCES "MedicalStores" ("MedicalStoreId") ON DELETE CASCADE
);

-- Indexes for ChemistPayoutAccounts
CREATE UNIQUE INDEX IF NOT EXISTS "IX_ChemistPayoutAccounts_MedicalStoreId" ON "ChemistPayoutAccounts" ("MedicalStoreId");
CREATE INDEX IF NOT EXISTS "IX_ChemistPayoutAccounts_RazorpayLinkedAccountId" ON "ChemistPayoutAccounts" ("RazorpayLinkedAccountId");

-- =====================================================
-- SECTION 6.3: CHEMIST ACTIVATION FEE TABLE + MedicalStore.ActivatedOn
-- (EF migration: 20260609140523_AddChemistActivationPayment)
-- One-time onboarding fee (₹14,999 + GST) collected via Razorpay Payment Links.
-- =====================================================

-- Add activation timestamp to MedicalStores (anchors the 30-day-free fee window)
ALTER TABLE "MedicalStores" ADD COLUMN IF NOT EXISTS "ActivatedOn" timestamp with time zone NULL;

CREATE TABLE IF NOT EXISTS "ChemistActivationPayments" (
    "Id" integer GENERATED BY DEFAULT AS IDENTITY,
    "MedicalStoreId" uuid NOT NULL,
    "Amount" numeric(10,2) NOT NULL,
    ...
    "Status" character varying(20) NOT NULL,
    "CreatedOn" timestamp with time zone NOT NULL,
    "PaidOn" timestamp with time zone NULL,
...
CREATE INDEX IF NOT EXISTS "IX_ChemistActivationPayments_RazorpayPaymentLinkId" ON "ChemistActivationPayments" ("RazorpayPaymentLinkId");

-- =====================================================
-- SECTION 6.4: PAYMENT SPLIT TABLE + Order bill/convenience columns
-- (EF migration: 20260609171446_AddPaymentSplitAndOrderBreakup)
-- Phase 2 — splits each captured order payment between chemist + Pharmaish.
-- =====================================================

-- Order amount breakdown used by the platform-fee split
ALTER TABLE "Orders" ADD COLUMN IF NOT EXISTS "BillAmount" numeric(10,2) NULL;
ALTER TABLE "Orders" ADD COLUMN IF NOT EXISTS "ConvenienceFee" numeric(10,2) NULL;

CREATE TABLE IF NOT EXISTS "PaymentSplits" (
    "Id" integer GENERATED BY DEFAULT AS IDENTITY,
    "OrderId" integer NOT NULL,
    "RazorpayPaymentId" character varying(100) NOT NULL,
    "TotalCaptured" numeric(10,2) NOT NULL,
    "BillAmount" numeric(10,2) NOT NULL,
    "ConvenienceFee" numeric(10,2) NOT NULL,
    "PlatformFee" numeric(10,2) NOT NULL,
    "ChemistAmount" numeric(10,2) NOT NULL,
    "PharmaishAmount" numeric(10,2) NOT NULL,
    "RazorpayTransferId" character varying(50) NULL,
    "ChemistLinkedAccountId" character varying(50) NULL,
    "TransferStatus" character varying(20) NOT NULL,
    "CreatedAt" timestamp with time zone NOT NULL,
    CONSTRAINT "PK_PaymentSplits" PRIMARY KEY ("Id"),
    CONSTRAINT "FK_PaymentSplits_Orders_OrderId" FOREIGN KEY ("OrderId") REFERENCES "Orders" ("OrderId") ON DELETE CASCADE
);

-- Indexes for PaymentSplits
CREATE INDEX IF NOT EXISTS "IX_PaymentSplits_OrderId" ON "PaymentSplits" ("OrderId");
...
SELECT 'ChemistPayoutAccounts' as TableName, COUNT(*) as RecordCount FROM "ChemistPayoutAccounts";
SELECT 'ChemistActivationPayments' as TableName, COUNT(*) as RecordCount FROM "ChemistActivationPayments";
SELECT 'PaymentSplits' as TableName, COUNT(*) as RecordCount FROM "PaymentSplits";
