...
class _DeliveryProfilePageState extends State<DeliveryProfilePage> {
  ...
  static const Color _theme = Colors.black;
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      AppLogger.error('DeliveryProfile error: $e');
  ...
  Future<void> _fetchFromApi(String userId) async {
    try {
      ...
      final r = await http.get(
        Uri.parse('${AppConstants.apiBaseUrl}/DeliveryBoys/user/$userId'),
  ...
  Widget _buildError() => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      ElevatedButton.icon(
        ...
        style: ElevatedButton.styleFrom(backgroundColor: _theme, foregroundColor: Colors.white),
      ),
    ]),
  );
  ...
  Widget _buildBody() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.delivery_dining, size: 38, color: _theme),
            ),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(fullName.isNotEmpty ? fullName : 'Delivery Boy',
                  style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 6),
              _roleBadge('Delivery Boy', Icons.local_shipping),
              const SizedBox(height: 6),
        ...
        // ── Delivery Info (only if API returned data) ──
        if (_deliveryBoyId.isNotEmpty || _vehicleType.isNotEmpty || _licenseNumber.isNotEmpty) ...[
          _section('Delivery Information', Icons.delivery_dining, [
            if (_deliveryBoyId.isNotEmpty) _row(Icons.badge_outlined, 'Delivery Boy ID', _deliveryBoyId),
            if (_vehicleType.isNotEmpty)   _row(Icons.two_wheeler,    'Vehicle Type',    _vehicleType),
            if (_licenseNumber.isNotEmpty) _row(Icons.credit_card,    'License Number',  _licenseNumber),
          ]),
          const SizedBox(height: 16),
        ],
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              style: OutlinedButton.styleFrom(
                ...
                side: const BorderSide(color: _theme, width: 1.5),
  ...
  Widget _section(String title, IconData icon, List<Widget> children) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(
        ...
        child: Row(children: [
          Icon(icon, color: _theme, size: 20),
          const SizedBox(width: 8),
          Text(title, style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: _theme)),
        ]),
      ),
      Padding(
        padding: const EdgeInsets.all(16),
